...
const NavLinks = ({ handleClick }) => (
  <nav className="flex flex-col lg:flex-row mt-8 w-full px-2 lg:px-32 gap-6">
    {links.map((item) => (
      <NavLink
        key={item.name}
        to={item.to}
        className={({ isActive }) => {
          return (
            "flex flex-col justify-center items-center font-outfit text-2xl md:text-xl text-center w-full h-40 px-6 rounded-lg border border-bunker-gray-500 bg-bunker-gray-500 hover:border-riptide-accent hover:border hover:shadow-card-shadow hover:transition-all hover:ease-in hover:duration-200 " +
            (isActive
...
const Home = () => {
  ...
  return (
    <div
      className="h-full lg:h-screen lg:overflow-hidden font-outfit "
